...
class BST:
    ...
    def DeepAllNodes(self, order):
        return tuple(self.go_deep(order, self.Root))

    def go_deep(self, order, node):
        if node is None:
            return []

        if order == 0:
            return self.go_deep(order, node.LeftChild) + [node] + self.go_deep(order, node.RightChild)
        if order == 1:
            return self.go_deep(order, node.LeftChild) + self.go_deep(order, node.RightChild) + [node]
        if order == 2:
            return [node] + self.go_deep(order, node.LeftChild) + self.go_deep(order, node.RightChild)